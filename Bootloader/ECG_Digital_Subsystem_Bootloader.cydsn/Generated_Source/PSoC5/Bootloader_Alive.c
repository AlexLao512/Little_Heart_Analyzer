...
/*******************************************************************************
* Function Name: Bootloader_Alive_Start
********************************************************************************
*
* Summary:
...
void Bootloader_Alive_Start(void) 
{
    ...
    Bootloader_Alive_CLKEN |= Bootloader_Alive_CLKEN_MASK;
	Bootloader_Alive_CLKSTBY |= Bootloader_Alive_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Bootloader_Alive_Stop
********************************************************************************
*
* Summary:
...
void Bootloader_Alive_Stop(void) 
{
    ...
    Bootloader_Alive_CLKEN &= (uint8)(~Bootloader_Alive_CLKEN_MASK);
	Bootloader_Alive_CLKSTBY &= (uint8)(~Bootloader_Alive_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Bootloader_Alive_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Bootloader_Alive_CLKEN & Bootloader_Alive_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Bootloader_Alive_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Bootloader_Alive_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Bootloader_Alive__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Bootloader_Alive_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Bootloader_Alive_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Bootloader_Alive_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Bootloader_Alive_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Bootloader_Alive_GetSourceRegister();
    uint16 oldDivider = Bootloader_Alive_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Bootloader_Alive_CLKEN & Bootloader_Alive_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Bootloader_Alive_DIV_PTR, clkDivider);
                Bootloader_Alive_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Bootloader_Alive_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Bootloader_Alive_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Bootloader_Alive_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Bootloader_Alive_DIV_PTR, clkDivider);
				Bootloader_Alive_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Bootloader_Alive_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Bootloader_Alive_DIV_PTR);
}


/*******************************************************************************
* Function Name: Bootloader_Alive_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Bootloader_Alive_MOD_SRC |= modeBitMask & (uint8)Bootloader_Alive_MODE_MASK;
}


/*******************************************************************************
* Function Name: Bootloader_Alive_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Bootloader_Alive_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Bootloader_Alive_MODE_MASK));
}


/*******************************************************************************
* Function Name: Bootloader_Alive_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Bootloader_Alive_MOD_SRC & (uint8)(Bootloader_Alive_MODE_MASK);
}


/*******************************************************************************
* Function Name: Bootloader_Alive_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Bootloader_Alive_GetDividerRegister();
    uint8 oldSrc = Bootloader_Alive_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Bootloader_Alive_MOD_SRC |= CYCLK_SSS;
        Bootloader_Alive_MOD_SRC =
            (Bootloader_Alive_MOD_SRC & (uint8)(~Bootloader_Alive_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Bootloader_Alive_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Bootloader_Alive_MOD_SRC & Bootloader_Alive_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Bootloader_Alive_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Bootloader_Alive_PHASE = clkPhase & Bootloader_Alive_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Bootloader_Alive_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Bootloader_Alive_PHASE & Bootloader_Alive_PHASE_MASK;
}
